...
def get_weight(shape, gain=np.sqrt(2), use_wscale=False, lrmul=1):
    print('[get_weight] shape: {}, gain: {:.3f}, lrmul: {}'.format(shape, gain, lrmul))
...
def dense(x, fmaps, **kwargs):
    print('[dense] fmaps: {}, kwargs: {}'.format(fmaps, kwargs))
    if len(x.shape) > 2:
...
def conv2d(x, fmaps, kernel, **kwargs):
    print('[conv2d] fmaps: {}, kernel: {}, kwargs: {}'.format(fmaps, kernel, kwargs))
...
def upscale2d_conv2d(x, fmaps, kernel, fused_scale='auto', **kwargs):
    print('[upscale2d_conv2d] fmaps: {}, kernel: {}, fused_scale: {}, kwargs: {}'.format(fmaps, kernel, fused_scale, kwargs))
    assert kernel >= 1 and kernel % 2 == 1
...
def style_mod(x, dlatent, **kwargs):
    print('[style_mod] dlatent: {}, kwargs: {}'.format(dlatent, kwargs))
...
def apply_noise(x, noise_var=None, randomize_noise=True):
    print('[apply_noise] noise_var: {}, randomize_noise: {}'.format(noise_var, randomize_noise))
    assert len(x.shape) == 4  # NCHW
...
def minibatch_stddev_layer(x, group_size=4, num_new_features=1):
    print('[minibatch_stddev_layer] group_size: {}, num_new_features: {}'.format(group_size, num_new_features))
...
def G_style(latents_in, is_training=False):
    truncation_psi = 0.7
    truncation_cutoff = 8
    ...
    num_layers = dlatents.get_shape().as_list()[1]
...
def G_mapping(latents_in, latent_size=512, dlatent_size=512, mapping_layers=8, mapping_fmaps=512):
    print()
    print('G_mapping')
    print()
    dtype = tf.float32
...
def nf(stage):
    fmap_base, fmap_decay, fmap_max = 8192, 1.0, 512
    return min(int(fmap_base / (2.0 ** (stage * fmap_decay))), fmap_max)


def blur(x):
    return blur2d(x, [1, 2, 1])


def G_synthesis(dlatents_in, lod_in, dlatent_size=512, resolution=1024, ):
    print()
    print('G_synthesis')
    print()
    ...
    def block(res, x):  # res = 3..resolution_log2
        ...
        with tf.variable_scope('%dx%d' % (2 ** res, 2 ** res)):
            with tf.variable_scope('Conv0_up'):
                x = layer_epilogue(blur(
                    upscale2d_conv2d(x, fmaps=nf(res - 1), kernel=3, gain=gain, use_wscale=use_wscale,
                                     fused_scale=fused_scale)), res * 2 - 4)
            with tf.variable_scope('Conv1'):
                x = layer_epilogue(conv2d(x, fmaps=nf(res - 1), kernel=3, gain=gain, use_wscale=use_wscale),
                                   res * 2 - 3)
            return x

    def torgb(res, x):  # res = 2..resolution_log2
        lod = resolution_log2 - res
        print('[torgb] res: {}, lod: {}'.format(res, lod))
